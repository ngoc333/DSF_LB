...
namespace FORM
{
    public partial class FRM_PROD_STATUS_YEARLY_V2 : Form
    {
        public FRM_PROD_STATUS_YEARLY_V2()
        {
            InitializeComponent();
        }
        ...
        public DataTable SMT_PROD_YEARLY_SELECT(string ARG_TYPE, string ARG_DATE, string ARG_LINE_CD, string ARG_MLINE_CD)
        {
            ...
            try
            {
                string process_name = "MES.PKG_SMT_LONGTHANH.SMT_PROD_YEARLY_SELECT";
        ...
        private void BindingData()
        {
            ...
            {
                //formatband();
                bandYear.Caption = DateTime.Now.ToString("yyyy");
                string curr_month = DateTime.Now.ToString("MM");
        ...
        private void InitChart(string sChart, DataTable dt, DevExpress.XtraCharts.ChartControl chartControl)
        {
            try
            {
                ...
                chartControl.DataSource = dt.Select("DAY <>'AVG'", "YMD").CopyToDataTable();
        ...
        {
            lblTitleProd.Text = "Yearly Assembly Production Average: 0%";
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                try
                {
                    ...
                    lblTitleProd.Text = "Yearly Assembly Production Average: " + num.ToString() + "%";
        ...
        private void FRM_PROD_STATUS_DAILY_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible)
            {
                ...
                switch (lang)
                {
                    case "Vi":
                        lang = "Vi";
                        lbltitle.Text = "Sản xuất trong năm";
                        break;
                    default:
