...
namespace FORM
{
    public partial class FRM_SMT_OSD_INTERNAL_PHUOC : Form
    {
        public FRM_SMT_OSD_INTERNAL_PHUOC()
        {
            InitializeComponent();
            timer1.Stop();
        }
        ...
        public FRM_SMT_OSD_INTERNAL_PHUOC(string Caption, int indexScreen, string Line_cd, string Mline_cd, string Lang)
        {
            InitializeComponent();
            timer1.Stop();
        ...
        private DataTable SEL_DATA_OSD(string ARG_TYPE,string ARG_FROM,string ARG_TO)
        {
            ...
            MyOraDB.Process_Name = "MES.PKG_SMT_LONGTHANH.SMT_OSD_INTERNAL";
        ...
        private void BindingChart()
        {
            try
            {
                ...
                DataTable dt = SEL_DATA_OSD(Mline+ "M", DATE_FROM, DATE_FROM);
                chartNosew.DataSource = null;
                chartHF.DataSource = null;
                chartPrinting.DataSource = null;
                chartLaser.DataSource = null;
                ...
                InitDataChart("UPR",chartLaser, dt);
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (cCount >= 40)
            {
                try
                {
                    this.Cursor = Cursors.WaitCursor;
                    string DATE_FROM = UC_MONTH.GetValue() + "01", DATE_TO = UC_MONTH.GetValue() + DateTime.DaysInMonth(Convert.ToInt32(UC_MONTH.GetValue().Substring(0, 4)), Convert.ToInt32(UC_MONTH.GetValue().Substring(4, 2))).ToString();
                    loadTopLeft(SEL_DATA_OSD("CHART1"+Mline, DATE_FROM, DATE_TO)); 
                    bindingDataGrid(DATE_FROM, DATE_TO);
                    BindingChart();
                    this.Cursor = Cursors.Default;
                    cCount = 0;
        ...
        private void gvwBase_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            ...
            {
                dtF = SEL_DATA_OSD("POPUP1" + Mline, dtf.Columns[e.Column.ColumnHandle].ColumnName.ToString().Replace("'", "").Replace(" ", "").Replace(",", ""),
            ...
            {
                dtF = SEL_DATA_OSD("POPUP2" + Mline, dtf.Columns[e.Column.ColumnHandle].ColumnName.ToString().Replace("'", "").Replace(" ", "").Replace(",", ""),
                                       dtf.Columns[e.Column.ColumnHandle].ColumnName.ToString().Replace("'", "").Replace(" ", "").Replace(",", ""));
